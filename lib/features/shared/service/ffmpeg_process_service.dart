...
class FfmpegProcessService {
  ...
  String patternIn(String dir, String ext) => p.join(dir, 'frame_%05d.$ext');
  ...
  Future<double?> probeDurationSeconds(File input) async {
    ...
    try {
      final ffprobePath = await _ffmpegHelper.ffprobePath;
      if (ffprobePath != null) {
    ...
    // Fallback to ffmpeg
    try {
      final r = await _ffmpegHelper.runFFmpeg(['-i', input.path]);
      final out = '${r.stderr}${r.stdout}';
      final m = RegExp(r'Duration:\s*(\d+):(\d+):(\d+\.?\d*)').firstMatch(out.toString());
      if (m != null) {
  ...
  Future<FfmpegResult> runWithProgress({
    ...
    try {
      ...
      return FfmpegResult(output: null, exitCode: -1, error: 'FFmpeg error: $e');
    }
  }
  ...
  Future<FfmpegResult> runLoose({
    ...
    try {
      ...
      return FfmpegResult(output: null, exitCode: -1, error: 'FFmpeg error: $e', outputDir: outputDir);
    }
  }
}